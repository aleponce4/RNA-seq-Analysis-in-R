...
top_pc1_genes <- names(sort(abs(pc1_loadings), decreasing = TRUE))[1:100] # Top 100 genes for PC1
...
top_forest_genes <- head(importance_df_sorted, 100)
...
# Create a new dataframe with random forest and PCA genes
top_genes_df <- data.frame(
  PCA_Genes = c(top_pc1_genes, rep(NA, max_length - length(top_pc1_genes))),
  Forest_Genes = c(top_forest_genes, rep(NA, max_length - length(top_forest_genes)))
)

# K-means clusters genes
# Subset the data for clusters 2, 4, and 5
cluster_2 <- subset(gene_clusters, Cluster == 2)$Gene
cluster_4 <- subset(gene_clusters, Cluster == 4)$Gene
cluster_5 <- subset(gene_clusters, Cluster == 5)$Gene

# Make all vectors the same length (fill with NA for shorter ones)
cluster_2 <- c(cluster_2, rep(NA, max_length - length(cluster_2)))
cluster_4 <- c(cluster_4, rep(NA, max_length - length(cluster_4)))
cluster_5 <- c(cluster_5, rep(NA, max_length - length(cluster_5)))

# Add new columns to the dataframe
top_genes_df$K_Means_Cluster_2 <- cluster_2
top_genes_df$K_Means_Cluster_4 <- cluster_4
top_genes_df$K_Means_Cluster_5 <- cluster_5
